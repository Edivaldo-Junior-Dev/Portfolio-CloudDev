...
// Mocks foram removidos. O backend DEVE estar rodando.

import { User } from '../types';

const API_URL = '/api'; // Usando proxy do Vite para evitar CORS e facilitar config
...
export const api = {
  ...
  login: async (email: string, password: string): Promise<{ user: User, token: string }> => {
    try {
      // Nota: O endpoint no server.js é /auth/login, mas o proxy redireciona /api/auth se configurado assim.
      // Vamos assumir que o proxy redireciona '/api' -> 'http://localhost:3001'
      // Então chamaremos '/api/auth/login' que vira 'http://localhost:3001/auth/login'
      ...
      throw new Error(e.message || "Erro de conexão com o servidor. Verifique se o backend está rodando.");
    }
  },
